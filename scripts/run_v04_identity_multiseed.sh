#!/usr/bin/env bash
# Train one seed across all three v0.4 identity methods on one GPU.
# Usage: scripts/run_v04_identity_multiseed.sh <GPU_ID> <SEED>
set -euo pipefail
...
CONFIG_ROOT="$ROOT_DIR/configs/v04_identity"
RUN_ROOT="$ROOT_DIR/runs/v04_identity"
LOG_ROOT="$ROOT_DIR/logs/v04_identity"
METHODS="identity_class_001 identity_branch_001 output_contrast_001"
...
if ! flock -n 9; then
  echo "v04 seed$SEED queue is already running" >&2
  exit 3
fi
...
path = Path(f"runs/v04_identity/seed{os.environ['SEED']}.queue_status.json")
...
mcfg = config["model"]
expected = sum(v.numel() for v in SiblingRestormer(
    dim=int(mcfg["dim"]),
    blocks_per_level=tuple(mcfg["blocks_per_level"]),
    heads=tuple(mcfg["heads"]),
    projection_dim=int(mcfg["projection_dim"]),
    degradation_conditioned=bool(mcfg.get("degradation_conditioned", False)),
    identity_mode=mcfg.get("identity_mode", None),
    identity_source_count=int(mcfg.get("identity_source_count", 71)),
).parameters())
assert int(summary["parameters"]) == expected, (summary["parameters"], expected)
