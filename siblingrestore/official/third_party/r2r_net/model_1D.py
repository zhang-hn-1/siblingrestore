...
import time
from net.feature_bank_1D import DegradationMemory
...
class R2R(nn.Module):
    def __init__(self, opt,  img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1], key_dim=64, value_dim=512, is_train=True, stage=0, deg_type=["denoise_15"], train_mode="pretrain"):
        super().__init__()
        self.encoder = NAFKeyEncoder(width=width, middle_blk_num=middle_blk_num, enc_blk_nums=enc_blk_nums)
        self.mem_encoder = NAFMemEncoder(img_channel,  deg_channel=1, width=width, middle_blk_num=middle_blk_num, enc_blk_nums=[2, 2, 4, 8])
        ...
        self.dm = DegradationMemory(opt, T_max=64, key_dim=key_dim, value_dim=value_dim, deg_types=deg_type)
    ...
    def forward(self, inp, clean=None, ids=None, interact_label=None):
        ...
        if self.is_train and self.train_mode == "pretrain":
            ...
            mem_feat = self.mem_encoder(mem_frame)
            ...
            assert len(list(set(gt_ids))) == 1
            gt_ids = torch.tensor(gt_ids, device=ids.device)
            if gt_ids[0] == 0:
                self.dm.update_bank("denoise", mk, mv)
            elif gt_ids[0] == 1:
                self.dm.update_bank("derain", mk, mv)
            elif gt_ids[0] == 2:
                self.dm.update_bank("dehaze", mk, mv)
            elif gt_ids[0] == 3:
                self.dm.update_bank("deblur", mk, mv)
            elif gt_ids[0] == 4:
                self.dm.update_bank("lowlight", mk, mv)
            read_out = self.dm.get_deg_prompt(qk)
        else:
            if self.is_train:
                gt_ids = []
                for id in ids:
            ...
            read_out = self.dm.get_deg_prompt(qk_, interact_label=interact_label)
        ...
        if self.is_train and self.train_mode == "finetune":
            return x_restore, gt_ids
        return x_restore
...
class R2RTest(nn.Module):
    def __init__(self, ckpt_path, prompts_path, prompts_name, deg_type, train_mode="finetune"):
        super().__init__()
        net = R2R(opt=None, is_train=False, deg_type=[deg_type], train_mode=train_mode)
        state_dict = torch.load(ckpt_path+deg_type+"/"+prompts_name+".ckpt")['state_dict']
        state = {}
        for key in state_dict.keys():
            state[key[4:]] = state_dict[key]
        net.load_state_dict(state, strict=True)
        net.dm.load_prompts(epoch=prompts_name, save_root=prompts_path+deg_type, deg_type=deg_type)
        self.net = net
    def forward(self, x, interact_label=None):
        return self.net(x, interact_label=interact_label)

class R2RLocal(Local_Base, R2RTest):
    def __init__(self, train_size=(1, 3, 224, 224), fast_imp=False,ckpt_path="", prompts_path="", prompts_name="last", deg_type="denoise", train_mode="finetune"):
        Local_Base.__init__(self)
        R2RTest.__init__(self,ckpt_path, prompts_path, prompts_name,deg_type, train_mode=train_mode)
