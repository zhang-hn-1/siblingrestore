...
from net.feature_bank_3D import DegradationMemory
...
class R2R(nn.Module):
    def __init__(self, opt=None,  img_channel=3, width=32, middle_blk_num=1, enc_blk_nums=[1, 1, 1, 28], dec_blk_nums=[1, 1, 1, 1], key_dim=64, value_dim=512, is_train=True, stage=0, num_classes=3, train_mode="pretrain"):
        ...
        self.key_proj = KeyProjection(width * 16, keydim=key_dim)

        self.key_comp = nn.Conv2d(width * 16, value_dim, kernel_size=3, padding=1)

        self.value_proj = ValueProjection(width * 16, valuedim=key_dim)

        self.value_comp = ValueComp(width * 16, valuedim=value_dim)
