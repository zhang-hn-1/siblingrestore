...
class LayerNorm3D(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.normalized_dim == 'C':
            x_perm = x.permute(1, 2, 3, 0)
            x_norm = self.layer_norm(x_perm)
            x_out = x_norm.permute(3, 0, 1, 2)
        else:
            x_perm = x.permute(0, 2, 3, 1)
            x_norm = self.layer_norm(x_perm)
            x_out = x_norm.permute(0, 3, 1, 2)
...
class DegradationMemory(nn.Module):
    def __init__(self,opt, T_max=32, key_dim=64, value_dim=512, denoise_bank_key=None, denoise_bank_value=None, derain_bank_key=None, derain_bank_value=None, dehaze_bank_key=None, dehaze_bank_value=None):
        super().__init__()
        self.deg_types = ["denoise", "derain", "dehaze"]
        self.T_max = T_max
    ...
    def update_bank(self, deg_type, mk, mv):
        if deg_type == "denoise":
            ...
            mk = torch.cat(k_list, dim=0)

            v_list = [mv for _ in range(n)]
            mv = torch.cat(v_list, dim=0)
            ...
            for i in range(mk.shape[0]):
                if len(self.denoise_bank_key) < self.T_max:
                    ...
                    key_combined = torch.cat(shuffled_key, dim=0).unsqueeze(0).permute(0, 2, 1, 3, 4)
                    value_combined = torch.cat(shuffled_value, dim=0).unsqueeze(0).permute(0, 2, 1, 3, 4)
                    self.denoise_bank_key.clear()
                    self.denoise_bank_value.clear()
                    update_keys = self.update_denoise_key_3Dconv(key_combined).squeeze(0).permute(1, 0, 2, 3)
                    update_values = self.update_denoise_value_3Dconv(value_combined).squeeze(0).permute(1, 0, 2, 3)
            ...
            for i in range(mk.shape[0]):
                if len(self.derain_bank_key) < self.T_max:
                    ...
                    update_keys = self.update_derain_key_3Dconv(key_combined).squeeze(0).permute(1, 0, 2, 3)
                    update_values = self.update_derain_value_3Dconv(value_combined).squeeze(0).permute(1, 0, 2, 3)
            ...
            for i in range(mk.shape[0]):
                if len(self.dehaze_bank_key) < self.T_max:
                    ...
                    update_keys = self.update_dehaze_key_3Dconv(key_combined).squeeze(0).permute(1, 0, 2, 3)
                    update_values = self.update_dehaze_value_3Dconv(value_combined).squeeze(0).permute(1, 0, 2, 3)
    ...
    def comprehensive_attention_processing(self,
        ...
        """Class-aware memory attention for 3D prompt retrieval."""
        ...
        mk_flat = mk.view(n_total, -1)
        qk_flat = qk.view(batch_size, -1)
        mv_flat = mv.view(n_total, -1)
        ...
        scores = torch.stack([scores_n1, scores_n2, scores_n3], dim=1)
    ...
    def common_attention(self,mk, qk, mv, n1, n2, n3,):
        """Standard soft retrieval over the full memory bank."""
        device = mk.device
        batch_size = qk.shape[0]
        n_total = n1 + n2 + n3
        h, w = qk.shape[2], qk.shape[3]
        ...
        mk_norm_sq = torch.sum(mk_flat ** 2, dim=1)
        dot_product = torch.matmul(qk_flat, mk_flat.t())
    ...
    def contrastive_divergence_loss(self, feature_lists, temperature=0.07):
        """Contrastive-style divergence loss for prompt banks."""
        all_features = []
        labels = []
        ...
        features_norm = F.normalize(all_features, dim=-1, eps=1e-6)
        sim_matrix = torch.matmul(features_norm, features_norm.T) / temperature
