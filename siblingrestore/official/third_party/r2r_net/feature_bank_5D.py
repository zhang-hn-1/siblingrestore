...
class DegradationMemory(nn.Module):
    ...
    def get_deg_prompt(self, qk, interact_label=None):
        ...
        if len(self.denoise_bank_key) == 0 or len(self.derain_bank_key) == 0 or len(self.dehaze_bank_key) == 0 or len(self.deblur_bank_key) == 0 or len(self.lowlight_bank_key) == 0:
            # Empty memory: upstream returns int 0 which crashes downstream
            # F.interpolate; return a zero prompt tensor of the query shape so
            # the rest of the forward graph can still execute.
            return None, torch.zeros_like(qk)
        mk = torch.cat([torch.cat(self.denoise_bank_key, dim=0), torch.cat(self.derain_bank_key, dim=0), torch.cat(self.dehaze_bank_key, dim=0), torch.cat(self.deblur_bank_key, dim=0), torch.cat(self.lowlight_bank_key, dim=0)], dim=0)

        mv = torch.cat([torch.cat(self.denoise_bank_value, dim=0), torch.cat(self.derain_bank_value, dim=0), torch.cat(self.dehaze_bank_value, dim=0), torch.cat(self.deblur_bank_value, dim=0), torch.cat(self.lowlight_bank_value, dim=0)], dim=0)

        n1 = len(self.denoise_bank_key)
        n2 = len(self.derain_bank_key)
        n3 = len(self.dehaze_bank_key)
        n4 = len(self.deblur_bank_key)
        n5 = len(self.lowlight_bank_key)


        label, readout = self.comprehensive_attention_processing(mk, qk, mv, n1, n2, n3, n4, n5, interact_label=interact_label)
    ...
    def comprehensive_attention_processing(self,
        ...
        def get_class_scores(method):
            ...
            if method == 'max':
                return [
                    ...
                    similarity[:, n1 + n2+n3:n1+n2+n3+n4].max(dim=1).values,
                    similarity[:, n1 + n2+n3+n4:].max(dim=1).values
                ]
                ...
                return [
                    ...
                    similarity[:, n1 + n2+n3:n1+n2+n3+n4].mean(dim=1),
        ...
        class_masks[3, n1 + n2+n3:n1+n2+n3+n4] = True
        class_masks[4, n1+n2+n3+n4:] = True
